...
class AuthController extends Controller
{
    ...
    public function register($request, $response)
    {
        ...
        $validation = $this->container->validator->validate($request,[
            'name' => v::notEmpty()->alpha()->length(3),
        ...
        $now = new \DateTime(date('h/m/Y H:i:s'));
        $now->modify('+1 hour');
    ...
    public function confirmation($request, $response)
    {
        ...
        if (strtotime(date('d/m/Y H:i:s')) > strtotime($user->confirmation_expires)) {
            $this->container->flash->addMessage('error', 'Tempo para confirmação esgotado. clique <a href="' . $this->container->router->pathFor('auth.resend') . '?email=' . $user->email . '">aqui</a> para reenviar outra confirmação!');
        }
        else
        {
